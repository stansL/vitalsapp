...
public class AdultPatientsPageController {
	...
	public void get(PageModel model, UiUtils ui,
        ...
        Function<Patient, PatientWrapper> wrapperFunction = p -> {
            ...
            PatientWrapper wrapper = new PatientWrapper(p);
            wrapper.setObsList(obsList);
            return wrapper;
        };
        List<PatientWrapper> wrapperList = allPatients.stream().filter(p -> p.getAge() > 15).map(wrapperFunction).collect(Collectors.toList());
